...
async function getOrCreateQueue() {
    ...
    if (!queue) { queue = new Queue(); await queue.save(); }
    return queue;
}
...
app.post('/api/auth/signup', async (req, res) => {
    try {
        ...
        if (existingUser) return res.status(400).json({ message: "Username pehle se hi exist karta hai!" });
...
app.post('/api/auth/patient-checkin', async (req, res) => {
    try {
        ...
        const myAssignedToken = queue.totalTokensDistributed < queue.currentToken 
            ? queue.currentToken 
            : queue.totalTokensDistributed;
...
io.on('connection', async (socket) => {
    ...
    try {
        const queue = await getOrCreateQueue();
        const patients = await Patient.find().sort({ tokenNumber: 1 });
        socket.emit('queue-updated', { currentToken: queue.currentToken, totalTokensDistributed: queue.totalTokensDistributed, patients });
    ...
    socket.on('admin-edit-patient', async (data) => {
        try {
            ...
            await Patient.updateOne(
                ...
                { name: newName, tokenNumber: parseInt(newTokenNumber), mobileNumber: newMobileNumber }
            );
            await broadcastQueueStatus(); 
        } catch (err) { console.error(err); }
    });
